...
class ContinuousSignalPlotter:
    ...
    def add_signal(self, expr_str, label=None, period=None):
        ...
        # ✅ Configurar automáticamente el evaluador numérico y el nombre actual
        local_dict = self._get_local_dict()
    ...
    def _extract_impulses(self):
        ...
        for term in expr_terms:
            ...
            for delta in deltas:
                arg = delta.args[0]
                var_sym = self.var_symbols[self.current_name]
                roots = sp.solve(arg, var_sym)
                amp = term.coeff(delta)
                var_sym = self.var_symbols[self.current_name]
                d_arg = sp.diff(arg, var_sym)
                scale = sp.Abs(d_arg)

                for r in roots:
                    try:
                        scale_val = float(scale.subs(var_sym, r))
                        amp_eval = amp.subs(var_sym, r).doit().evalf()
    ...
    def plot_convolution(self, x_name, h_name, num_points=None, show=True):
        """
        ...
        # Register the convolution under the name "y"
        if self.expr is None:
            self.signal_defs["y"] = sp.sympify(0)  # Placeholder for numerical case
        else:
